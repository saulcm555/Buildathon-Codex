...
import { isLocalMergeMode, mergeMcpClient } from './services/mcpClient';
...
export default function App() {
  ...
  async function decide(decision: Decision) {
    ...
    try {
      ...
      setNotice(decision === 'accepted' ? (isLocalMergeMode ? 'Propuesta aplicada al archivo local tras validar su hash.' : 'Propuesta validada. La demo pública no modifica archivos ni GitHub.') : decision === 'edited' ? 'Edición guardada para el historial.' : 'Propuesta rechazada; no se modificó ningún archivo.');
    } catch (error) { setNotice(errorMessage(error)); } finally { setBusy(false); }
  }
  ...
  return <main className="shell">
    ...
    {!analysis ? <section className="hero">
      ...
      <section className="proposal card"><div className="proposal-title"><div><p className="eyebrow">PROPUESTA DE FUSIÓN</p><h2>Una solución que conserva ambos cambios</h2></div><button className="ghost" onClick={regenerate} disabled={busy}>↻ Nueva propuesta</button></div><p className="explanation">{analysis.explanation}</p><textarea value={proposal} onChange={(event) => setProposal(event.target.value)} spellCheck="false" aria-label="Código de la propuesta" />
        ...
        <div className="actions"><button className="ghost danger" onClick={() => decide('rejected')} disabled={busy}>Rechazar</button><button className="ghost" onClick={() => decide('edited')} disabled={busy}>Guardar edición</button><button className="primary" onClick={() => decide('accepted')} disabled={busy}>{isLocalMergeMode ? 'Aceptar y aplicar' : 'Validar propuesta (demo)'} <span>→</span></button></div>
      </section>
