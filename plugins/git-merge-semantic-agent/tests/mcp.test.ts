...
describe("MCP tools", () => {
  it("analyzes, generates, and applies a human-approved proposal", async () => {
    ...
    const conflictId = JSON.parse(analyzed.content[0]!.text).conflictId as string;
    const generated = await client.callTool({ name: "generate_merge_proposal", arguments: { conflictId } });
    const proposalId = JSON.parse(generated.content[0]!.text).id as string;
    ...
    expect(JSON.parse(appliedResult.content[0]!.text)).toEqual({ applied: true, filePath: "src/example.ts" });
